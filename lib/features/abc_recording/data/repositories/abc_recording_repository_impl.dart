...
class AbcRecordingRepositoryImpl implements AbcRecordingRepository {
  ...
  @override
  Future<Either<Failure, List<RecordingSession>>> getSessionsByPatient(String patientId) async {
    try {
      final models = await remoteDataSource.getSessionsByPatient(patientId);
      final entities = models.map((model) => model.toEntity()).toList();
      return Right(entities);
    } on ServerException catch (e) {
      return Left(ServerFailure(e.message));
  ...
  @override
  Future<Either<Failure, RecordingSession>> createRecordingSession(RecordingSession session) async {
    try {
      final model = RecordingSessionModel.fromEntity(session);
      final result = await remoteDataSource.createRecordingSession(model);
      return Right(result.toEntity());
    } on ServerException catch (e) {
      return Left(ServerFailure(e.message));
    } on NetworkException catch (e) {
